...
make_EHelper(popa) {
  if (decoding.is_operand_size_16) {
    rtl_pop(&t1); reg_w(R_DI) = t1;
    rtl_pop(&t1); reg_w(R_SI) = t1;
    rtl_pop(&t1); reg_w(R_BP) = t1;
    rtl_pop(&t1); /* SP - discard */
    rtl_pop(&t1); reg_w(R_BX) = t1;
    rtl_pop(&t1); reg_w(R_DX) = t1;
    rtl_pop(&t1); reg_w(R_CX) = t1;
    rtl_pop(&t1); reg_w(R_AX) = t1;
  } else {
    rtl_pop(&cpu.edi);
    rtl_pop(&cpu.esi);
    rtl_pop(&cpu.ebp);
    rtl_pop(&t0);  // discard ESP
    rtl_pop(&cpu.ebx);
    rtl_pop(&cpu.edx);
    rtl_pop(&cpu.ecx);
    rtl_pop(&cpu.eax);
